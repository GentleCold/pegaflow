...
def test_cross_layer_standard_attention_nhd_uses_token_block_dimension():
    info = _infer_kv_cache_registration(
        FakeTensor(
    ...
    assert info.layout == "blocks-first"
    assert info.num_blocks == 3
    assert info.bytes_per_block == 2 * 93 * 2 * 64 * 1 * 128 * 2
    assert info.physical_blocks_per_logical_block == 2


def test_cross_layer_standard_attention_hnd_uses_token_block_dimension():
    info = _infer_kv_cache_registration(
        FakeTensor(
            shape=(6, 1, 93, 2, 64, 128),
            stride=(
                1 * 93 * 2 * 64 * 128,
                93 * 2 * 64 * 128,
                2 * 64 * 128,
    ...
    assert info.bytes_per_block == 1 * 93 * 2 * 64 * 128 * 2
    assert info.physical_blocks_per_logical_block == 1
